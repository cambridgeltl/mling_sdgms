...
from nn_model.semicldc_model import SEMICLDCModel, calc_au
from utils.logpdfs import multi_diag_normal

import pdb

const = np.float128((sympy.log(2 * sympy.pi)).evalf(64))

class XLSEMICLDCModel(SEMICLDCModel):
  def __init__(self, params, data_list, model_dict = None):
    super(XLSEMICLDCModel, self).__init__(params, data_list, model_dict = model_dict)
  ...
  def forward_trainenc(self, src_batch_in, src_batch_lens, src_batch_lb, src_batch_ohlb,
    ...
    # warm up
    if self.warm_up:
      src_loss_dict = defaultdict(float)
      src_L_pred = None
      if self.src_le > .0 and src_batch_in is not None:
